...
async fn main() -> std::io::Result<()> {
    ...
    let server = HttpServer::new(move || {
        App::new()
            ...
            // .configure(legacy::homeassistant::configure_routes) // MIGRATED to integration handlers
            // .configure(legacy::weather::configure_routes) // MIGRATED to integration handlers
            ...
            // .configure(legacy::system::configure_routes) // MIGRATED to integration handlers
            // .configure(legacy::mqtt::configure_routes) // MIGRATED to integration handlers
            .configure(legacy::slack::configure_routes)
            // .configure(legacy::security::configure_routes) // MIGRATED to new architecture
            .configure(legacy::database::configure_routes)
            // .configure(legacy::health::configure_routes) // MIGRATED to integration handlers
            // NOTE: All high-priority + medium-priority modules now handled by new architecture
