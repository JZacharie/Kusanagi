    ...
    pub async fn process_message(&self, message: A2UIMessage) -> Result<()> {
        match message {
            A2UIMessage::SurfaceUpdate { surface_id, components } => {
                let mut surface = self.repository.get_surface(&surface_id).await?
            ...
            A2UIMessage::DataModelUpdate { surface_id, data } => {
                let mut data_model = self.repository.get_data_model(&surface_id).await?
                    .unwrap_or_default();
                ...
                self.repository.save_data_model(&surface_id, data_model).await?;
            }
            A2UIMessage::UserAction { surface_id, action_id, component_id, payload: _ } => {
                tracing::info!("User Action received: {} on component {} for surface {}", action_id, component_id, surface_id);
