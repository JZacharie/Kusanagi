...
// use crate::legacy:: // Disabled for core version{argocd, cluster, events, nodes, backups, chat_storage, mcp, slack};
use kube::Client;
...
async fn query_ollama(prompt: &str) -> Result<String, String> {
    let client = reqwest::Client::builder()
        .timeout(std::time::Duration::from_secs(300)) // Increased to 300 seconds
        .build()
