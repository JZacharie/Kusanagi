...
async fn fetch_vps_metrics(client: &reqwest::Client) -> (f64, f64, f64) {
    ...
    let cpu_value = query_prometheus_scalar(client, &url, cpu_query).await.unwrap_or(0.0);
    ...
    let disk_value = query_prometheus_scalar(client, &url, disk_query).await.unwrap_or(0.0);
    ...
    let net_value = query_prometheus_scalar(client, &url, net_query).await.unwrap_or(0.0);
    
    (cpu_value.max(0.0), disk_value.max(0.0).min(100.0), net_value.max(0.0))
}
...
async fn fetch_gpu_and_energy_metrics(client: &reqwest::Client) -> (f64, f64, f64, f64, f64) {
    ...
    let gpu_util = query_prometheus_scalar(client, &url, gpu_util_query).await.unwrap_or(0.0);
    
    // DCGM_FI_DEV_GPU_TEMP - GPU temperature
    let gpu_temp_query = r#"avg(DCGM_FI_DEV_GPU_TEMP)"#;
    let gpu_temp = query_prometheus_scalar(client, &url, gpu_temp_query).await.unwrap_or(0.0);
    
    // DCGM_FI_DEV_POWER_USAGE - GPU power usage
    let gpu_power_query = r#"avg(DCGM_FI_DEV_POWER_USAGE)"#;
    let gpu_power = query_prometheus_scalar(client, &url, gpu_power_query).await.unwrap_or(0.0);
    ...
    (gpu_util.max(0.0), gpu_temp.max(0.0), gpu_power.max(0.0), solar_prod, house_cons)
}
